...
const createMask = (width: number, height: number, radius: number) => {
  ...
  const indent = radius * (1 + mr) < largestSide * 2 ? radius * (1 + mr) : largestSide * 2
...
export const squirclify = (el: HTMLElement, r: number) => {
  if (!el) return
